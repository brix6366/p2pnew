...
<nav class={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md' : 'bg-transparent'}`}>
  <div class="container mx-auto px-4 py-4">
    <div class="flex items-center justify-between">
      ...
      <div class="hidden md:flex items-center space-x-8">
        <Link to="/" class="text-neutral-800 hover:text-primary-600 transition">Home</Link>
        <Link to="/search" class="text-neutral-800 hover:text-primary-600 transition">Find Cars</Link>
        <Link to="/how-it-works" class="text-neutral-800 hover:text-primary-600 transition">How it Works</Link>
        {#if $authStore.isAuthenticated && $authStore.user}
          <div class="relative">
            <button class="flex items-center space-x-2 text-neutral-800 hover:text-primary-600 transition">
        ...
        {:else}
          <Link to="/login" class="text-neutral-800 hover:text-primary-600 transition">Sign in</Link>
          <Link to="/register" class="btn btn-primary">Register</Link>
        {/if}
      </div>
      
      <!-- Mobile menu button -->
      <button on:click={toggleMenu} class="md:hidden text-neutral-800 focus:outline-none">
  ...
  <!-- Mobile menu -->
  {#if isMenuOpen}
    <div class="md:hidden bg-white border-t animate-enter" transition:slide>
